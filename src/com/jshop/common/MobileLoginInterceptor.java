 ...
 * 
 */
public class MobileLoginInterceptor extends MethodFilterInterceptor implements
		Serializable {
	...
	protected String doIntercept(ActionInvocation invocation) throws Exception {
		...
		if (StringUtils.isEmpty(request.getParameter("result"))) {
			request.setAttribute("seo", seoDao.getShopEntityByUrl(
					request.getServletPath(), namespace, request));
		}
*/
		if (includePackages.indexOf(namespace) != -1) {
			Object loginer = null;
			Map session = ctx.getSession();
			loginer = session.get(Global.SESSION_KEY_MOBILE_LOGINER);
