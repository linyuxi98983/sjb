define(function (require, exports, module) {
	var $ = require('jquery');	
	SJB.ActionCollection.init(jQuery('.photo_box_new .btn_all .collection'));
    SJB.ActionAsk.init(jQuery('.photo_box_new .btn_all .question'));
    SJB.downloadPic.init('.photo_box_new .download');
	//点击浏览按钮打开幻灯片
	...
	var newPhotoBox = {
		...
		questionList:function(){
			...
			if(arrayRightObj[_this.pid] && len != 0){
	            ...
	            jQuery('.reels_msg .questions').append(str);	
			}
			else{
				jQuery('.reels_msg .questions').hide();
			}
			...
			*/
			...
			if(arrayObj[_this.p]['user'][_this.i].indentityCN == 'yz'){
				...
				*/
				...
				//预约设计
				jQuery('.btn_reservation').unbind('click');
		...
		bindEvent:function(){
			...
			jQuery('.photo_box_new .arrow_right').bind('click',function(){
				...
				_this.pid = jQuery(this).attr('data-id');
				_this.fillContent();
