/*省市区选择*/
    function checkForm() {
        ...
        if(!selArea && $("#areaSqm").length > 0)
        {
            alert('请填写项目面积');
            $("#areaSqm").focus();
            return;
        }
        if(isNaN(selArea) && $("#sqm").length > 0)
        {
            alert('项目面积只能填写数字');
            $("#sqm").focus();
            return;
    ...
    $('#zxd-form-submit').click(function(e) {
        ...
        if(checkForm())
        {
            ...
            tel = $('#zxd-tel').val(),
            selArea = $("#getArea").val(),
            yusuan = $('#select-itme-yusuan').text(),
            referer = $("#referer").val();
	    	...
	    	$.ajax({
             type: "POST",
             url: "/sjb/mobile/demand/demand!save.action?result=json",
             data: {
            	 "entity.nickname":$("#zxd-name").val(),
            	 "entity.phoneNo":$("#zxd-tel").val(),
            	 "entity.areaSqm":$("#getArea").val(),
            	 "entity.budgetName":$('#select-itme-yusuan').val()
             },
             dataType: "json",
            ...
            /*$.post('/sjb/mobile/demand/demand!save.action',{
            	'zbtype':zbType,'project_type':project_type,
            	'sourceid':id,'s_sourceid':sid,'phone':tel,'chenghu':name,
            	'oarea':selArea,'yusuan':yusuan,'biao':biao,
