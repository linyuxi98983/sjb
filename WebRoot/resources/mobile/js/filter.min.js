var bizFilter = {
    ...
    bindBackClick: function() {
        ...
        c = this; - 1 != a.indexOf("ddcategory.php") ? $.cookie("_last_jumping_url", a, {
            expires: 1
        }) : this.validRedirectLoc(a) && $.cookie("_last_jumping_url", a, {
            expires: 1
        });
        $("a.search_back, a.xph_back").click(function() {
            b == $.cookie("_last_jumping_url") ? null != a ? (a = c.addSid(a), window.location.href = c.validRedirectLoc(a) ? a: c.addSid(ddTouchIndex)) : window.location.href = c.addSid(ddTouchIndex) : $.cookie("_last_jumping_url") ? window.location.href = c.addSid($.cookie("_last_jumping_url")) : window.location.href = c.addSid(ddTouchIndex)
        })
    },
    ...
    initDom: function() {
        ...
        for (var b in this.radioArr)"undefined" != typeof a[this.radioArr[b]] && 1 == a[this.radioArr[b]] && $("#" + b).addClass("on");
    ...
    bindCancelClick: function() {
        ...
        $("#clear_btn").click(function() {
            ...
            $("#lp").val("");
            $("#hp").val("");
            for (var b in a.radioArr) $("#" + b).removeClass("on");
    ...
    bindSubmitClick: function() {
        ...
        $("#submit_btn").click(function() {
            ...
            a.getSelectedAtt();
            var c = [],
            e = "is_stock is_promo is_outlets is_mvip is_presell is_ddself".split(" ");
            for (d in b) if (a.canceled) {
                if ("keyword" == d || "sid" == d || "req" == d || "cid" == d) c[d] = b[d]
            } else $.inArray(d, e) && 0 == b[d] || (c[d] = b[d]);
            for (d in a.params) c[d] = a.params[d];
            "undefined" == typeof a.params.city && "undefined" != typeof c.city && delete c.city;
            "undefined" == typeof a.params.brand && "undefined" != typeof c.brand && delete c.brand;
            "undefined" != typeof c.brand && 0 == c.brand.length && delete c.brand;
            for (var d in a.radioArr) 0 == c[a.radioArr[d]] && delete c[a.radioArr[d]];
            "" != $("#lp").val() ? c.lp = $("#lp").val() : delete c.lp;
            "" != $("#hp").val() ? c.hp = $("#hp").val() : delete c.hp;
            b = "?";
            for (d in c)"att" != d && ("cid" == d && (c[d] = c[d].replaceAll("_", ".")), "brand" == d && $.isArray(c[d]) && (c[d] = c[d].join(",")), b += d + "=" + c[d] + "&");
    ...
    bindRadioClick: function() {
        var a = this;
        $("div.filtrate_has>ul>li").click(function() {
            var b = $(this).hasClass("on") ? 0 : 1;
            $(this).toggleClass("on");
            var c = $(this).attr("id");
            a.params[a.radioArr[c]] = b
        })
    },
    ...
    getAttNameStr: function(a) {
        ...
        a.find("li").each(function() {
            ...
            if ($(this).hasClass("on") || a.hasClass("on")) null != a.attr("cname") ? b.push(a.attr("cname")) : b.push(a.attr("aname"))
        });
        a = b.join(",");
    ...
    getOtherAttsStr: function() {
        ...
        for (b in this.otherAtts) {
            var c = b.replaceAll("'", "");
            if ($.isArray(this.otherAtts[b])) for (var e in this.otherAtts[b]) {
                var d = c + ":" + this.otherAtts[b][e]; - 1 == a.indexOf(d) && a.push(d)
            }
        }
    ...
    bindGetSubCategories: function() {
        ...
        $("#filter_category_list").click(function(c) {
            ...
            var f = e.attr("cid");
            a.params.cid = f;
            if (e.hasClass("filtrate_list_li_a") && e.hasClass("open")) e.removeClass("open"),
            "undefined" != typeof a.bizCateHtml[f] && (e.next("ul").remove(), d || (a.removeAllAtts(), $(a.bizAttHtml[f].brand).insertBefore(b), $(a.bizAttHtml[f].att).insertAfter(b), a.params.brand = "", a.otherAtts = [], a.bindAttToggle(), a.bindAttClick(), a.brandPartShow()));
            else if (e.hasClass("filtrate_list_li_a") && !e.hasClass("open")) {
                if ("undefined" == typeof f || !f) return ! 1;
                c = a.getQuerystringObj();
                c.keyword && (c.keyword = decodeURIComponent(c.keyword));
                c.cid = f.replaceAll("_", ".");
                c.act = "get_sub_categories";
                if ("undefined" != typeof a.bizCateHtml[f]) return e.next("ul").remove(),
                e.addClass("open"),
                $(a.bizCateHtml[f]).insertAfter(e),
                d || (a.removeAllAtts(), $(a.bizAttHtml[f].brand).insertBefore(b), $(a.bizAttHtml[f].att).insertAfter(b), a.params.brand = "", a.otherAtts = [], a.bindAttToggle(), a.bindAttClick(), a.brandPartShow()),
                !0;
                $.ajax({
                    type: "GET",
                    ...
                    success: function(c) {
                        if (null != c) {
                            var g = "",
                            h = a.getAttsHtml(c.att, f);
                            0 == c.category.length ? a.params.cid = f: (e.addClass("open"), g = a.getCategoriesHtml(c.category, e), e.next("ul").remove(), $(g).insertAfter(e));
                            d || (a.removeAllAtts(), $(h.brand).insertBefore(b), $(h.att).insertAfter(b), a.params.brand = "", a.otherAtts = [], a.bizCateHtml[f] = g, a.bizAttHtml[f] = h, a.bindAttToggle(), a.bindAttClick(), a.brandPartShow())
                        }
                    }
                })
            }
    ...
    getCategoriesHtml: function(a, b) {
        ...
        var c = "<ul>",
        e = parseInt(b.css("padding-left").replace("px", ""));
        if ("undefined" != typeof a) for (i in a) c += '<li class="filtrate_list_li"><a style="padding-left:' + (e + 20) + 'px" class="filtrate_list_li_a" cid="' + a[i].cid + '" cname="' + a[i].cname + '" href="javascript:;">' + a[i].cname + "</a></li>";
        return c + "</ul>"
    },
    getAttsHtml: function(a, b) {
        ...
        if ("undefined" != typeof a) for (i in a) {
            var d = "",
            f = a[i].items;
            c.push(a[i].id);
            var k = 1 == a[i].id ? "\u5168\u90e8\u5c55\u5f00": "\u5168\u90e8",
            d = d + ('<div class="filtrate_category" att_id="' + a[i].id + '"><a class="filtrate_category_a" href="javascript:;">' + a[i].name);
            if (0 == f.length) d += "</a></div>";
            else {
                d += '<span class="up_down">' + k + "</span></a></div>";
                d += '<ul class="clearfix filtrate_address filtrate_list att_item" id="filter_' + a[i].id + '_list">';
                for (j in f) d += '<li><a href="javascript:;" cid="' + f[j].id + '" aname="' + f[j].name + '">' + f[j].name + "</a></li>";
                d += "</ul>"
            }
            1 == a[i].id ? e.brand = d: e.att += d
        }
        this.currentAtts = c;
        this.atts[b] = c;
        return e
    ...
    getAsocArrSize: function(a) {
        var b = 0;
        for (key in a) a.hasOwnProperty(key) && b++;
        return b
    }
};