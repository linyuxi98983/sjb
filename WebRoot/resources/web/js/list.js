		...
		$.ajax({
			...
			success: function(message) {
				if(message.success == true || message.success == 'true'){
					...
					if(url && url.indexOf('list.action') == -1){
						$('#listForm').submit();
					}else{
						window.location.reload(true);
					}
				}else{
					alert('删除失败'+message.errorMsg);
				}
			}
		});
	});
...
// 保存列表排序
function saveListOrderNo(tableName){
	var o = $("input[name='orderNo']");
	var orderNo = [];
	o.each(function(){
		orderNo.push('orderNo=' + $(this).attr('cid')+'_'+$(this).val());
	});
	orderNo.push('tableName='+tableName);
	$.ajax({
		url: "web/common/control!updateListOrderNo.action",
		type: "POST",
		data:  orderNo.join('&'),
		dataType: "json",
		cache: false,
...
// 更新字段
function updateField(module,data,callback){
	$.ajax({
		url: "web/"+module+"/"+module+"!update.action",
		type: "POST",
		data: data,
		dataType: "json",
		cache: false,
