   ...
   <div id="success_msg" style="display: none;"><s:property value="msg"/></div>
	...
	<form id="listForm" action="web/designDemand/designDemand!list.action" method="post">                 
			...
			<tbody><tr>
				...
				<th>
					<span><span><s:if test="state==2">操作</s:if><s:if test="state==4">状态</s:if></span></span>
				</th>
			</tr>
			<s:iterator value="list" status="statu" >   
				<tr>
					...
					<td>
						...
						<!-- 已指定设计师 -->
						<s:if test="state==2">
							<a href="pc/org/demand/demand!save.action?id=<s:property value="id"/>&state=1">[同意]</a>
							<a href="pc/org/demand/demand!save.action?id=<s:property value="id"/>&state=2">[拒绝]</a>
						</s:if>
					</td>
				</tr>
				</s:iterator>
